...
class Industri extends Controller
{
    ...
    public function filter (Request $request){
        ...
        if (Auth::check() != null ) return View('home.industri', ['industri' => $industri, 'user' => $this->repository->getData(), 'teks' => $teks]);
        return View('home.industri', ['industri' => $industri, 'teks' => $teks]); 
        }
        $katakunci = $request->kk;
         $industri = DB::table('industri')
        ->where('nama', 'like', '%'.$katakunci.'%')
