...
class AdminPenempatan extends Controller
{
    ...
    public function editSiswa(Request $request){
        ...
        $penempatan = DB::table('penempatan')
        ->join('pengajuan','penempatan.kd_pengajuan','=','pengajuan.kd_pengajuan')
        ->select('pengajuan.kd_industri')->where('kd_penempatan', $request->kd_penempatan)->first();
        $industri = Industri::find($request->industri);
        $count = Pengajuan::where('kd_industri', $request->industri)->where('status', 'Diterima')->count();
        if($count < $industri->kuota || $industri->kd_industri == $penempatan->kd_industri){
            ...
            $pengajuan->save();
            return back()->with('success','Penempatan berhasil diubah!');
        } else return back()->withErrors('Kuota Instansi sudah penuh!');
        return back();
    }
    ...
    public function searchSiswa(Request $request){
        ...
        if($request->has('q')){
            ...
            ->join('kelas','siswa.kd_kelas', '=', 'kelas.kd_kelas')->select('nis','siswa.nama','kelas.nama as kelas')->where('siswa.nama', 'LIKE', '%'. $request->q .'%')->get();
